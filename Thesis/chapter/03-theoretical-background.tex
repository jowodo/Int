...
\td{IR around page 45;
REFLECTIVITY? can also be used for determination of thickness p47\cite{Dumin1967}
}
\td{QUESTIONS: 
...
\paragraph{(Fourier Transform) Infrared spectrometer}
 ...
 In the two-beam-spectrometer the light is split, sent through the sample and a reference and alternately sent through a monochromator to a detector (thermocouple). 
%
...
How the two beams will interfere upon recombination depends on the optical path difference (also called retardation) of the two light beams.
...
When the refractive index of two layers differ, \gls{ir} can be used to measure the thickness\cite{Dumin1967}.
