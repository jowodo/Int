...
The target acts as anode and the substrate (holder) as cathode (see \td{figure \ref{fig:sputter}}).
...
"Multivariate Adaptive Regression Splines"\cite{friedman1991mutlivariate}. 
The \texttt{CRAN} package which implements \gls{mars} is called \texttt{earth} due to \gls{mars} being trade marked\cite{mars}.
%by the \texttt{earth} package (available from CRAN). 
%The name \texttt{earth} was chosen as \texttt{mars} has been trade marked\cite{mars}. 
