...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Statistics vs Regression}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
they can be divided into supervised (there is a known solution) and unsupervised (exploratory).  
...
%One which is really popular now a days is neural networks (NN) and deep learning. 
...
and the weights (parameters), with which the data is routed from one neuron to another, 
are optimised during training. 
%
%There are of course many more \gls{ml} designs for different tasks. 
%Although, mainly \gls{nn} are in the media and take up a lot of space in the field 
%DL shines when it is used for complex data with massive data sets. 
Convolutional-layer neural networks excel in picture recognition\cite{Lecun1995conv} and in quantum mechanics too\cite{westermayr2020combining}.
%One architecture which distinguishes itself for picture recognition is a convolutional networks.\td{cite some}
Other common architectures are linear regression, kernel ridge regression, support vector regression.
There are also lesser know algorithms like \gls{ga} (part of evolutionary algorithms) and \gls{pso}. 
One advantage of the last two is that they start with a small data set
%don't require the massive amount of data like \gls{dl} 
and periodically request new data and try to solve the problem iteratively.
...
\textbf{things I should try if they work:}
\begin{itemize}
...
\begin{itemize}
    \item[( )] machine learning and diff fields in ML
    \item[( )] Genetical algorithm 
