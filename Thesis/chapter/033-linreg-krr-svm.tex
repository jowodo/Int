...
\end{equation}
%
\Gls{svm}s can handle non-perfect data by introducing a penalization for wrongly categorized samples. 
\begin{equation}
    L_H = sum_i^n max(0, 1- y_i ( \mathbf{w} \mathbf{x}_i + b ) 
\end{equation}
if the prediction and the true category don't agree, they have opposite signs and 
their product will be a negative number.
This is called a soft-margin \gls{svm} in contrast to the previously hard-margin \gls{svm}.

A big advantage of \gls{svm} is that through the kernel trick non-linearly separable data 
can be divided. 
