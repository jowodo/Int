...
\gls{db} velocity (\mmps{10}, \mmps{12}, \mmps{14}, \mmps{16}, \mmps{18} or \mmps{20}) were set, 
...
relative concentration $conc$, number of layers $layers$, calcination temperature $T_{cal}$[\oc{}], heating rate $v_{cal}$[\oc{}/\h{}], \gls{db} velocity $v_{DB}$[\mm{}/\s{}] and \gls{db} temperature $T_{DB}$[\oc{}], respectively. 
After testing the first samples, the \gls{db} velocities altered to 1 and 5 (see table \ref{tab:pre-opt1}).
\begin{table}[htb]
	\centering
...
\\
			...
			\hline
		\end{tabular}
		\subcaption{Hand picked experiments}
		\label{tab:pre-opt2}
	\end{subtable}
	\caption{}
	\label{tab:pre-opt}
...
\gls{ipo} because of much better soltion stability.
After 
