...
Thus maximizing the margin is equivalent with minimizing $\|\mathbf{w}\|$. 
...
\end{equation}
which can be rewritten by some mathematical wizardry (setting the partial derivatives of the lagrangian 
$\frac{\partial \mathcal{L}}{\partial \mathbf{w}}$ and $\frac{\partial \mathcal{L}}{\partial b}$ to zero and inserting into eq. (\ref{eq:svm03})): 
\begin{equation}
	\mathcal{L} = \sum_i^n \alpha_i - \frac{1}{2} \sum_i^n \sum_i^n \alpha_i \alpha_j y_i y_j \mathbf{x}_i \cdot \mathbf{x}_j 
\end{equation}
