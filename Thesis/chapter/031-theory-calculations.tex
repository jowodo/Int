...
Friedman presented \gls{mars} as an alternative to piecewise polynomials (splines) and local averaging methods (e.g. kernel functions), whose number of parameter quickly exceeds the practical for moderate dimensions (n>5). 
...
The main advancements are the ability to fit (possible complex) interactions and non-linearities. 
...
During the forward recursion basis functions are added in pairs until a certain number of terms is reached. 
Then, the backwards algorithm regularizes the function by removing terms. 
...
- "Choosing a previous term for multiplication (10) is analogous to choosing a (previous) region to split in (7). The optimization over 2, and t in (10) is quite similar to finding the optimal splitting variable and split point for partitioning a region. "\cite{friedman1988fitting}

\begin{equation}
    GCV(M) = \frac{1}{N} \sum_{i=1}^{N} \frac{ \left(y_i - \hat{f}(x_i) \right)^2 } {\left( 1- \frac{C(M)}{N}\right)^2 }
\end{equation}
with $M$ being the number of terms, $N$ the number of data points, $d$ the degrees of freedom and
\begin{equation}
    C(M) = (d + 1)M + 1
\end{equation}
...
%
- the error is due to unmeasured and uncontrolled 
...
The F-test was ingeniused by Ronald Fisher, an important figure in modern statistics. 
...
%This ratio is unaltered by adding constants to all observations or scaling errors. 
