...
\td{\Gls{pv} is cool and a big hope when trying to become carbon neutral as it uses the energy provided by sun directly in contrast to renewable energy sources (e.g. wind and water) or even carbon based sources. }
One sort of \gls{pv} is \gls{cigs} \cite{Vasekar2010}. % https://doi.org/10.1016/j.tsf.2009.09.033}
...
A non-toxic material which is suitable for this application is ZrO2. 
...
The method of doctor blading \td{(which is in principle a sol-gel process)} was chosen because of the availability to the industrial partner.
In order to optimize the resulting layer the multitude of parameters was optimized with an machine learning ansatz. 
...
\begin{figure}[htb]
	...
	\end{subfigure}
	\label{fig:sol}
	\caption{Aquatic and buthanolic solution in beaker glass with magnetic stir bars sealed with Parafilm} 
\end{figure}
%\fi
...
\begin{figure}
	...
	\caption{
		...
		The majority of the suction areas is sealed with tape to increase the under pressure in the remaining
	.}
	\label{fig:eric}
\end{figure}
...
NT2 had only 2 variables (heating rate and maximal temperature) instead of 4 (three heating rates and maximal temperature), where the \td{calcination time} was held constant.
