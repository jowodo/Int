...
\td{After setting the heating plate temperature, the vacuum plate temperature (see figure \ref{fig:eric}) and the \gls{db} velocity, the \gls{db} blade is set up and the sample placed on the vacuum plate. The vacuum is switched on and the blade is sent over the sample without liquid to see if it is held in position.}
