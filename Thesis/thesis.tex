...
This sol-gel process was chosen over spin coating because of the availability to the 
industrial partner.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PREOPTIMISATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\subsection{Preparatory Experiments/series (Pro-optimisation/Finding base process/Preliminary study)}
\subsection{Pre-optimisation}
...
allowed to hold the sample in place through underpressure and heat it to a certain 
temperature. The bounds of the process were then explored in a preliminary study using 
the Plackett-Burman \cite{Plackett1946} design implemented in the \texttt{python3} 
library \texttt{pyDOE}.
...
Some additional hand picked experiments were introduced to further narrow down the limits of 
...
\gls{ipo} because much better soltion stability.
After 
%a fitting recipe was found and 
...
How to describe one sample in one number? 
some kind of average
For every sample multiple \gls{iv} curves were measured. 
Each sample should be \td{characterized} by ideally one number. 
The average of each conductance would be \td{nahe liegend} but difficult to represent a 
sample correctly since the possible values span accross \td{größenordnungen}.
So the average of logarithms of values is the next obvious \td{attempt,idea,ansatz}.
