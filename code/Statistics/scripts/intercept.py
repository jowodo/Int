...
#
Y_emma_pred = np.mean(Y_emma, axis=0 )
Y_pre_pred = np.mean(Y_emma, axis=0)
Y_intra_pred = np.mean(Y_emma, axis=0)
Y_all_pred = np.mean(Y_emma, axis=0)
#
train_dataset = "complete data set" if args.fit_all else "EMMA data set"
# PRINT OUTPUT
if args.y_index == "0": 
    y_name = "all"
else: 
