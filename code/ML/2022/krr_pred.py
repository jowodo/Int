...
if args.scale : 
    ...
    print("./krr_pred.py -k %s -d %s -a %s -g %s -x" %(args.kernel, args.degree, args.alpha, args.gamma))
else:
    print("./krr_pred.py -k %s -d %s -a %s -g %s" %(args.kernel, args.degree, args.alpha, args.gamma))
...
print("MAE(emma): %.2f" %(MAE(Y_emma_pred,Y_emma)))
print("MSE(emma): %.2f" %(MSE(Y_emma_pred,Y_emma)))
Y_pre_pred=ML.predict(X_pre)
if args.scale : Y_pre_pred=ML.predict(sklearn.preprocessing.StandardScaler().fit_transform(X_pre))
print("MAE(pre): %.2f" %(MAE(Y_pre_pred,Y_pre)))
print("MSE(pre): %.2f" %(MSE(Y_pre_pred,Y_pre)))
...
print("MAE(inter): %.2f" %(MAE(Y_inter_pred,Y_inter)))
print("MSE(inter): %.2f" %(MSE(Y_inter_pred,Y_inter)))
Y_all_pred=ML.predict(X_all)
if args.scale : Y_all_pred=ML.predict(sklearn.preprocessing.StandardScaler().fit_transform(X_all))
print("MAE(all): %.2f" %(MAE(Y_all_pred,Y_all)))
print("MSE(all): %.2f" %(MSE(Y_all_pred,Y_all)))
if args.predall: 
    X = X_all
    Y = Y_all
else:
