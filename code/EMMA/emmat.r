debugging in: emmatn(t, tm, graph = graph, opt = opt, weight = weight, na = na, pr.mut = pr.mut) 
debug: {
    xpop <- x$xpop
    ypop <- x$ypop
    xspace <- x$xspace
    ...
    if (t == 1) {
        ...
        for (i in 1:length(Pb)) {
            if (fit.Pb[i] < fit[i]) 
                Pb.new <- c(Pb.new, Pb[i])
            if (fit[i] <= fit.Pb[i])
                Pb.new <- c(Pb.new, sam.x[i]) 
        }
    d <- distance(xpop, xspace, yspace, weight, opt)
    if (t == 1) {
        d.pop <- d$fit[tested]
    ...
    for (i in 1:ncol(xspace)) {
        if (i == 1) 
            dif2 <- xspace[Gb, i] - xspace[sam.x, i]
        if (i > 1) 
            dif2 <- cbind(dif2, xspace[Gb, i] - xspace[sam.x, i])
    }
    ...
    if (!is.null(fn1)) 
        ypop <- fn1(xpop)
    if (!is.null(fn2)) 
        ypop <- cbind(ypop, fn2(xpop))
    if (!is.null(fn3)) 
        ypop <- cbind(ypop, fn3(xpop))
    if (!is.null(fn4)) 
        ypop <- cbind(ypop, fn4(xpop))
    if (is.null(fn1)) {
        ypop <- NULL
        print("ADD THE MEASURED RESPONSE VALUES TO ypop")
    }
    structure(list(xpop = xpop, ypop = ypop, xspace = xspace, 
        yspace = yspace, opt = opt, nd = nd, na = na, tested = tested, 
        time = t, weight = weight, Gb = Gb, Pb = Pb, Gb.arch = Gb.arch, 
        Pb.arch = Pb.arch, v = v, sam.x = sam.x, add = add), 
        class = "emma")
}
