...
%%%%%%%%%%%%%%%%%% EXPERIMENTAL %%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\todo{materials section and sources (incl. purities is missing!) }
In this section the used chemicals, substrates, experimental procedures and any used lab equipment are described. 
%\td{
...
%\td{The experiments can be split into three sections: }
...
%\linebreak[4] 
(\cm{2.5}$\times$\cm{7.5}) from Sigma Aldrich, thinner, 
squared glass plates (\cm{2.5}\x\cm{2.5}) Menzel Gläser,
\gls{ito} coated glass (\cm{2.5}\x\cm{2.5}) from Sigma Aldrich with a sheet resistance of approximately 12 $\Omega$/sq, 
\gls{fto} coated glass (\cm{5}\x\cm{5}) from Sigma Aldrich and steel foil (10~cm~x~10~cm) provided by Sunplugged GmbH (\url{http://sunplugged.at/)}.
%
The steel foil was cut with a foil cutter into \cm{2.5}\x\cm{2.5} squares.
...
%, which is needed to measure the conductivity of the applied layer and \td{for \gls{sem} micrographs. (dont follow?)}
...
\begin{enumerate}
	\item \minutes{15} in \ml{50} \gls{water} and \ml{1} of Hellmanex III alkaline concentrate in an ultrasonic bath
	\item \minutes{15} in \gls{water} in an ultrasonic bath
	\item \minutes{15} in \gls{ipo} in an ultrasonic bath 
\end{enumerate}
%\td{is that the usual procedure for steel?}
...
These additives were added to influence pH and surface tension with the goal to influence the homogeneity of the resulting layer. %(in terms of what?)}. % \td{see Parvulescu2015}
...
%Unfortunately, this solution failed to produce \td{anything near homogeneous layers (??)}.
...
%\todo{what did you did you do percisely! It must be logical and should be followed}
Five different concentrations of the buthanolic solutions were prepared.
\ds{
\td{Not all (which?)} chemicals mentioned by Hu~et~al.\cite{Hu2016} were available, so chemically similar 
...
%The \td{\gls{1f}} was closest to the recipe proposed by Hu~et~al.~\cite{Hu2016}. 
...
%the \td{particle swarm optimization} started 
...
\td{precursor (transition metal complex \gls{zrpro}	\gls{vs} post-transition metal complex aluminium isopropoxide \ch{Al(PrO)3}), solvent (\gls{buoh} \gls{vs} 2-ethoxyethanol \ch{EtOEtOH}), reaction temperature 
...
\SI{500}{\celsius}).
( so everything is different!! now I'm confused? you dream up a recipe and optimize that? not how I would do that! also: quite a difference between alpro3 and zrpro4)}
}
%me          hu
%Zr(OPr)4    Al(iPrO)3   
%1-BuOH      2-MeO-EtOH  
...
added and stirred for another \minutes{\ds{10 to }15}. Finally, the stabilization 
...
%Most steel substrates (all pre-optimization and optimization samples) 
%All 
Steel substrates were doctor bladed with an Erichsen Coatmaster 510 film applicator with a heatable plate.
...
%The blade height was varied from \SI{0}{\milli\meter} to \SI{0.35}{\milli\meter} in \SI{0.05}{\milli\meter} steps and \td{did not substantially alter the results. (what do you mean?)}
...
and the \gls{db} speed (\mmps{10}, \mmps{12}, \mmps{14}, \mmps{16}, \mmps{18} or \mmps{20}), 
the blade was put into its initial position, the sample was placed on the vacuum plate and the vacuum 
was switched on. 
During pre-optimization, lower \gls{db} velocities (\mmps{0.1}, \mmps{0.5}, 
...
%'blade pusher' \td{film applicator, travel pusher, traverse something, pushing bar} 
was put into initial position, the blade was removed and excess solution was removed from the plate with a wipe. 
The coated substrate was dried for \minutes{5} on a \SI{200}{\celsius} heating plate.
%\td{transferred to the \SI{200}{\celsius} heating plate and rested on there for \minutes{5}. (why? what was the purpose?)}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Calcination of \ch{ZrO2}}
\label{sec:exp-calc}
A LabTech EH45C heating plate and a Naberterm LB410 muffle furnace were used to calcinate the coated samples. 
The heating plate could hold temperature for a certain amount of time, but heated with a fixed rate of \oc{10}/\minutes{}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CHARACTERISATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Characterization of \ch{ZrO2} layers}
...
\begin{figure}[tb]
	...
	(c) From the voltage source clockwise: variable voltage source, built-in amperometer, gold plated tungsten probe, sputtered aluminium contact, \gls{zro} layer to be measured, steel substrate, dried silver paste contact, gold plated tungsten probe. }%\td{(what is XXX circuit?)}}
\end{figure}
...
%Between sputtering and \gls{iv} measurements direct contacts to the steel substrate were created by removing a patch of the \gls{zro} layer with sand paper on two opposing edges of a sample and then by applying silver paste.
...
This resistance will be predominantly influence by the presence of shunts (hotspots or short-cuts between the top and bottom contact), appearing due to the inhomogeneous thickness, and pinholes through the ZrO2 layer. 

%\todo{ To me this does not follow any logical order!}
\section{Data Processing of Electrical Measurements}
\label{sec:eval}
...
%
%\todo{why did not i just take this?}
...
to the ideal non-conduction case ($g=10^{-13}$) was chosen instead of the average of logarithms and shall be called conductivity hereafter. % for lack of a better term. 
...
%\td{preopt of what? recipe?}
%\td{the choice of grenzen for \gls{doe} was done by biotic reinforcement learning (i.e.\ my brain)}
%\td{Screening}
%244 - 93 aqueous - 103 testing - 191 buthanol - 206 preopt - 244 emma
% 93 aq - 20 adapting Hu2016 - 88 testing process vars - 15 pre-opt - 38 emma (3 failed samples)
...
%The first 200 recipes were optimized via biotic refinforcement learning (i.e.\ my brain) in order to obtain reasonable constraints for computerized pre-optimization. \td{(xxx but using a completely untested recipe!)} 
%The main part of all samples (200 out of 244) were pre-optimization samples,  from which the bulk were screening experiments optimized via biotic reinforcement learning  (i.e.\ my brain) in order to obtain reasonable constraints for computerized pre-optimization. \td{(xxx but using a completely untested recipe!)} 
%Initially, glass plates were doctor bladed manually with aqueous solution and calcinated 
%via HP1. 
...
%otherwise the sample would charge and can not be pictured anymore. 
%this means if the produced layer is thick and homogeneous enough, it is difficult to see in sem
...
%Composition of the \td{aqueous solution (I thought this did not work?)}, heating program, blade distance (distance between blade and substrate) and layer count were varied but no homogeneous layers resulted. 
%
%The buthanolic recipe was introduced, which gave rise to homogeneous films.
%\td{After the blade moved over the substrate the remaining liquid needed about a minute to evaporate at room temperature. (2x mentioned this before!)}
%\td{
%The then current procedure resulted in clear and continuous films, but was plagued from inhomogeneities due to drying stains. 
%The drying stains could be circumvented by using a heat gun, but the results were not reproducible. 

%starting with pre-optimisation
%The glass plate of the film applicator was exchanged with a metal plate.
A metal plate with vacuum chuck and temperature control was used as a base in the film applicator. 
...
%
\ds{
table \ref{tab:pre-opt1}).
\begin{table}[hbt]
	\centering
	\caption{Pre-optimization experiments \td{(still smaller?? why? what am I supposed to learn?)} }
	\label{tab:pre-opt}
	...
	\begin{subtable}{0.5\linewidth}
		\centering
        \subcaption{Hand picked experiments; $^*$varied stabilization agent}
		\label{tab:pre-opt2}
		\begin{tabular}{cccccc}
	%conc	&layers	&vDOC	&TDOC	&vCal	&Tcal		\\
			\hline\hline
			$c_{zr}$	&$n_L$	&$v_{C}$	&$T_{C}$	&$v_{cal}$	&$T_{cal}$		\\
	%conc	&layers	&$v_{\textrm{C}}$	&$T_{\textrm{C}}$	&$v_{\textrm{cal}}$	&$T_{\textrm{cal}}$		\\
			\hline
	2	&8	&0.5	&40	&360	&470	\\
	2	&6	&2	&40	&360	&430	\\
	1	&4	&12	&70	&120	&500	\\
...
%	4$^\ddagger$	&6	&14	&60	&240	&500	\\
			...
			\hline
			\\ \\  \\
%			\multicolumn{6}{c}{*different stabilization reagent compositions}\\
		\end{tabular}
	\end{subtable}
\end{table}
}
...
%$T_{cal}$[\oc{}]
%$v_{cal}$[\oc{}/\h{}]
%Especially the doctor blading speed.
%}
...
%For the preliminary study a C machine and NT were used and the steel. 
...
\ml{1} \gls{acoh} (sample no.~199), \ml{0.5} \gls{acoh} plus \ml{0.5} \gls{ipo} (sample no.~201) and \ml{1} \gls{ipo} (sample no.~192).
%\td{As the sample showed the best all ex were proceeded with ipo as stabilization agent. }
...
%\td{After the boundaries of the solution were fixed }
...
%\td{plachett-burman design of experiment with conc, layers, calcination temperature, heating rate, C speed and C temperature as input variables.}
...
\begin{table}[ht!]
    ...
    \begin{tabular}{cccccc}
        \hline\hline
		$c_{zr}$	&$n_L$	&$v_{C}$	&$T_{C}$	&$v_{cal}$	&$T_{cal}$		\\
%        concentration   &layers &\gls{db} speed  &\gls{db} temperature   &heating rate   &calcination temperature    \\
        \hline
        2   &4  &10 &40 &120    &300    \\
        3   &6  &12 &50 &360    &400    \\
        4   &8  &14 &60 &600    &500    \\
