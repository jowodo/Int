...
Finally, the results of alternative regression methods \td{let loose on (language is too colloquial)} the data are compared with the \gls{emma} method. 
...
%\subsection{Solution}
%%%%%%%% AQUATIC 
As already indicated in the experimental section the aquatic recipe adopted from Anwar et al.~\cite{Anwar2017} failed to produce a \td{homogeneous layer. (of what?)}
...
Even after the consecutive application of two layers, there was no improvement and the results examined with \td{\gls{sem} (where is the evidence?)} reminded rather of a sparse crust than of a film. 
...
\todo{where is the evidence?}

%%%%%% STABILITY
As soon as the solution showed initial cloudiness, it was declared as unstable and used \td{no more for \gls{db}. (coating! DB = deutsche Bahn ... ist mir schon vorher aufgefallen)}
...
It therefore dependent on the water or oxygen in the air corresponding to an \td{oxidation or hyrolization (you're a chemist. so what is it?)} process.
%
...
%
\td{It was discovered that after a \gls{4f} \gls{acoh} solution has aged beyond cloudiness the addition 50\% of the volume in \gls{ipo} can even reverse the aging effect for a respectable amount of time. (but the conc.\ changed)}
...
%%%%%%%%% CROSS SECTION
Figure~\ref{fig:sem-cs1} shows the \td{cross section} of a layer of \gls{zro} on \gls{fto} glass at a deliberate fracture.
...
\begin{figure}
	...
	\caption{XRD spectra of steel substrate (yellow), \gls{zro} (blue), $\alpha$-Fe (green, idealized) and cubic-\gls{zro} (purple, idealized)\cite{gkatz1971xray} \td{(text in fig too small, use an offset, looks very amorphous to me)} }
	\label{fig:xrd}
...
%
The four largest peaks of cubic \gls{zro} at \td{$2\theta=30, 51, 60, 35$    (why? For what?)} (highest to lowest intensity) clearly stand out against the noise from the experimental \td{diffractogram. (spectrum: or 2theta and those numbers need a unit (grad))}  %can be clearly seen in the experimental spectra.
...
In figure~\ref{fig:ir-gt} \gls{uv}/\gls{vis}/\gls{nir} transmission spectra \td{of what?} can be seen. 
Each sample had different numbers of layers. 
The incident angle was 0 \td{degree} for each sample.
...
%
Figure~\ref{fig:ir-gr} shows reflectance spectra of \gls{uv}/\gls{vis}/\gls{nir} at an incident angle of 45 \td{degrees}. 
...
Especially, the \td{\gls{db}} velocity $v_{DB}$ (i.e. the velocity with which the blade moves and spreads the solution over the sample)
was varied and examined during pre-optimization. 
The slower the \td{\gls{db}} velocity, the more uniformly the solution evaporated. 
If the $v_{DB}$ was too slow (less than \mmps{1}), no \gls{zro} was formed. 
The reason is that the force exerted by the blade on the liquid is not strong enough to overcome \td{surface tension. (for buoh? cant believe this?)}
...
Additionally, the coating temperature affects the evaporation process together with the \gls{db} velocity $v_{DB}$. 
