...
In this section the used chemicals, substrates, experimental procedures and any used
\td{equipment (what do you mean?)} are described. 
...
is needed to measure the conductivity of the applied layer and \td{for \gls{sem} micrographs. (don't follow?)}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Aquatic Precursor Solutions}\label{sec:exp-sol-aq}
...
These additives were added to influence pH and surface tension with the goal to \td{influence the resulting layer (in terms of what?)}. % \td{see Parvulescu2015}
The \gls{water}-\gls{ipo} mixture was added to the \td{other solution (which? NOT CLEAR!)} and stirred over night. 
...
The other four solutions (\gls{2f}, \gls{3f}, \gls{4f} and \gls{5f}) were similar 
but with higher concentrations of \gls{zrpro} (see table \ref{tab:rec2})
with the aim of producing thicker layers.
%\td{p34}
\td{Different} solvents (butane-1,2-diol, \gls{buoh} and Propan-1-ol) and chelating agents 
(\gls{acac} and citric acid) were tested and later in the process - just before the 
\td{particle swarm optimisation} started - the stabilization compound (\gls{acoh}) was changed to \gls{ipo}.
...
After the layer was applied and dried, the tape was removed and the substrate heat treated \td{(at which T?)}.
...
\SI{0.05}{\milli\meter} steps and \td{did not substantially alter the results. (what do you mean?)}
...
underpressure could be applied to keep the substrate in place. 
Most of them were covered with tape to increase the suction intensity at the remaining ones \td{(see figure~\ref{fig:eric}).
...
The coated substrate was \td{transferred to the \SI{200}{\celsius} heating plate and rested on there for \minutes{5}. (why? what was the purpose?)}
...
fixed rate of \td{circa (and how do you optimze ca?)} \oc{10}/\minutes{}.
...
\td{All \gls{sem} micrographs were taken with a Zeiss Supra 40. (conditions? were samples sputtered etc...)}
...
\td{\Gls{xrd} diffractograms were obtained with a Thermo Scientific ARL Equinox 100 X-Ray Diffractometer. (where are the conditions?)}
...
\begin{figure}[htb]
	...
	(c) From the source clockwise: variable voltage source, built-in amperometer, gold plated tungsten probe, sputtered aluminium contact, \gls{zro} layer to be measured, steel substrate, dried silver paste contact, gold plated tungsten probe.\td{(what is XXX circuit?)}}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PREOPTIMISATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Pre-optimization}\label{sec:exp-preopt}
\td{preopt of what? recipe?}
%\td{the choice of grenzen for \gls{doe} was done by biotic reinforcement learning (i.e. my brain)}
%\td{Screening}
%The main part of the screening pre-optimization samples were optimized via biotic reinforcement learning (i.e. my brain). 
The main part of all samples (200 out of 244) were pre-optimization samples, 
from which the bulk were screening experiments optimized via biotic reinforcement learning 
(i.e.\ my brain) in order to obtain reasonable constraints for computerized pre-optimization. \td{(xxx but using a completely untested recipe!)} 
...
Composition of the \td{aquatic solution (I thought this did not work?)}, heating program, blade distance (distance between 
...
relative concentration of Zr $c_{zr}$, \td{number of layers $\lambda$ (layers of what?)}, calcination temperature $T_{cal}$[\oc{}], heating rate $v_{cal}$[\oc{}/\h{}], \gls{db} velocity $v_{DB}$[\mm{}/\s{}] and \gls{db} temperature $T_{DB}$[\oc{}], respectively.
...
%ooooh 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Shortly before the \gls{pso} the stabilization agent was changed from \todo{ \gls{acoh} to \gls{ipo} (but why?)} because of much better \td{solution stability. (how did yo test this??)}
...
showed \td{comparable passivization results (but where are they?)} and better stability of the solution. 
