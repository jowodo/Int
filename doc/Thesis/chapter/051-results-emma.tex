...
The most insulating recipes for each generation can be seen in table \ref{tab:emma-Gb}. 
...
lowest possible $c_{zr}$, $v_{C}$, $T_{C}$, $v_{cal}$, $T_{cal}$ and second highest possibel $\lambda$ (see table \ref{tab:input}). 
...
\begin{table}[htb]
	...
	\begin{tabular}{ccccccccc}
        \hline\hline
		generation& measurements &enr &$c_{zr}$ &$\lambda$ &$v_{C}$ &$T_{C}$ &$v_{cal}$ &$T_{cal}$\\
        \hline
...
The \td{green} points at generation number 0 indicate particles which were not included to be propagated. 
...
\td{Violet} lines connect individual particles in time. 
The \td{blue} line connects averages of particles of each generation (including initial non-propagated particles) and 
\td{yellow} lines connect averages of each generation of only those particles which were propagated. 
%
...
%
%    \label{eq:emma-phd1}
%    \hat{\rho}_1 &= -0.52 + 2.3\cdot 10^{-5} \cdot  T_{C}\cdot T_{cal} + 0.00080 \cdot h(6-\lambda)\cdot T_{cal}\\
%    \label{eq:emma-G1}
%    \hat{\gamma}_1 &= -26  + 0.0025 \cdot T_{C}\cdot T{cal}  +  0.056 \cdot  h(6-\lambda)\cdot T_{cal} \\
%    \label{eq:emma-layr1}
%    \hat{\lambda}_1 &= 7.1 + 8.1\cdot 10^{-5} \cdot T_{C}\cdot T_{cal} -  0.0056 \cdot  h(6-\lambda)\cdot T_{cal}\\
%    \label{eq:emma-vcal1}
%    \hat{v}_{cal,1} &= 19 - 0.00025 \cdot  T_{C}\cdot T_{cal} - 0.0055 \cdot  h(6-\lambda)\cdot T_{cal}\\
%
%    \label{eq:emma-phd2}
%    \hat{\rho}_2 &= 0.23\\
%    \label{eq:emma-G2}
...
%
    \label{eq:emma-phd4}
    \hat{\rho}_4 &=  -0.87 + 0.0047 \cdot  T_{cal} - 0.00036 \cdot  \lambda\cdot T_{cal}  +  0.0024 \cdot  h(\lambda-6)\cdot T_{C} \\
    \label{eq:emma-G4}
    \hat{\gamma}_4 &=     -19 + 0.28 \cdot  T_{cal}  - 0.022 \cdot  \lambda\cdot T_{cal}  +  0.16 \cdot  h(\lambda-6)\cdot T_{C} \\
    \label{eq:emma-layr4}
    \hat{\lambda}_4 &=  6.8 - 0.014 \cdot  T_{cal}  + 0.0018 \cdot  \lambda\cdot T_{cal}  + 0.0060 \cdot  h(\lambda-6)\cdot T_{C} \\
    \label{eq:emma-vcal4}
    \hat{v}_{cal,4} &=  29 - 0.052 \cdot  T_{cal}  + 0.0011 \cdot  \lambda\cdot T_{cal}  -  0.011 \cdot  h(\lambda-6)\cdot T_{C} 
\end{align}
...
This difference in magnitude fits the data well since the maxima of $\rho$ and $\gamma$ are separated by rough factor of 100 (see figure~\ref{fig:emma-gen} and appendix~\ref{sec:app-emma}). %, but the positive influence of the layer count $\lambda$ is counter intuitive 
...
The oppisite is the case, though. % (see figures \ref{fig:g-tcal} and \ref{fig:phd-tcal}).
...
Similarly, signs of coefficients of $\lambda \cdot T_{cal}$ and $h(\lambda -6) \cdot T_{C}$ correlate negatively in equations~(\ref{eq:emma-phd4})--(\ref{eq:emma-vcal4}) and the product of coefficients and input variables are again similar in order of magnitude. 
...
%From the analysis of coefficients it can be said, that the most important factor according to EMMA is $T_{cal}$, followed by $\lambda$, then $T_{C}$ and then the rest of the independent variables. 
The process variables $T_{cal}$ and $\lambda$ appear as \gls{bf}s in generation 3 and 4, whereas $v_{cal}$ only appears in generation 3's \gls{rf}s and $T_{C}$ only in generation 4's \gls{rf}s.
...
The \gls{epv} (data points per independent varible) are as low as 5 in the here presented optimization. % model and data acquisition method
...
The two independent variables $v_{cal}$ and $\lambda$ were included as \textit{optimizands} for two reasons: 
...
Moreover, $v_{cal}$ and $\lambda$ were only considered with 5\% weight in the overall objective function.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{rational behind including $v_{cal}$ and $\lambda$ as optimizands}
}
%
