...
%lowest solution concentration, second highest layer count, lowest \gls{db} speed and temperature, 
...
\begin{figure}[hb]
    ...
    \end{subfigure}
	\caption{
		(a)~calculated conductivity measures~$\gamma$ of each particle against the generation number %blue shows average for each generation and yellow shows average of each generation without discarded particles from initial generation. 
		(b)~calculated pinhole ratio~$\rho$ of each particle against the generation number %blue shows average for each generation and yellow shows average of each generation without discarded particles from initial generation. 
...
%as one would expect more layers to be more insulating and thus result in lower conductivity and a lower probability of holes going all the way trough the \gls{zro} layers. %pinhole ratio. 
...
calcination temperatures under \oc{400} do not suffice to produce compact coating layers (corresponding to $+h(400-T_{cal})$)
and 
...
Maximizing the calcination speed and minimizing the number of layer application iterations leads to minimization of process time.
...
Even if it did not influence the fit for the other splines, but it influences the choice of samples therefore it might have slowed down the process
