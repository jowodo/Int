...
The \td{best (what is "best"? How do you measure that?)} recipes for each generation can be seen in table \ref{tab:emma-Gb}. 
...
\begin{table}[htb]
	\centering
	\caption{Global best per generation with experiment numbers and experiment conditions}% \td{include units?}}
	\label{tab:emma-Gb}
...
%%% OVER TIME 
The only clear trend from table \ref{tab:emma-Gb} is the \td{layer count (again not yet clear)}, which rises with \td{the generations (which?)}. 
...
\end{table}
%
Contrarily, the predicted conducticity $\hat{\gamma}$ for each generation's \td{best (best in what?)}
...
%GENERATIONS
\td{How did you determine the pinhole densisty}
...
%
%    \label{eq:emma-phd1}
%    \hat{\rho}_1 &= -0.52 + 2.3\cdot 10^{-5} \cdot  T_{DB}\cdot T_{cal} + 0.00080 \cdot h(6-\lambda)\cdot T_{cal}\\
%    \label{eq:emma-G1}
%    \hat{\gamma}_1 &= -26  + 0.0025 \cdot T_{DB}\cdot T{cal}  +  0.056 \cdot  h(6-\lambda)\cdot T_{cal} \\
%    \label{eq:emma-layr1}
%    \hat{\lambda}_1 &= 7.1 + 8.1\cdot 10^{-5} \cdot T_{DB}\cdot T_{cal} -  0.0056 \cdot  h(6-\lambda)\cdot T_{cal}\\
%    \label{eq:emma-vcal1}
%    \hat{v}_{cal,1} &= 19 - 0.00025 \cdot  T_{DB}\cdot T_{cal} - 0.0055 \cdot  h(6-\lambda)\cdot T_{cal}\\
%
%    \label{eq:emma-phd2}
%    \hat{\rho}_2 &= 0.23\\
%    \label{eq:emma-G2}
...
\td{This (what is "this?)} fits the data well since the maxima of $\rho$ and $\gamma$ are separated by rough factor of 100 (see figure~\ref{fig:emma-gen} and appendix~\ref{sec:app-emma}). %, but the positive influence of the layer count $\lambda$ is counter intuitive 
...
%From the analysis of coefficients it can be said, that the most important factor according to EMMA is $T_{cal}$, followed by $\lambda$, then $T_{DB}$ and then the rest of the independent variables. 
