...
5 points before and 5 points after the origin with \td{at one point distance from zero} (which boils
...
where $C$ and $\varepsilon$ took the values 0.05--1 with step size 0.05 %, $\varepsilon$ 0.05--4.0 with 0.2 steps 
...
of the \gls{emma} data set were \td{scaled before each calculation.}
...
The score equals to the coefficient of determination $R^2$ which \td{ is defined the following for non linear regression functions:}
\begin{equation}
	\label{eq:r2}
