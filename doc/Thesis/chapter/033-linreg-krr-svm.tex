...
a plane in three dimensions) can fit data in regression problems or separate the data in classification problems without actually doing the transormation for every data point, which lowers computational costs.
...
%
The \gls{svm} was initially developed by Vladimir Vapnik for the binary classification of separable data, then improved to handle non-separable data and eventually adapted to solve regression problems.
...
\begin{equation}
	\label{eq:svm-soft}
	L_{SM} = \frac{1}{2} \|\mathbf{w}\|^2 + \sum_i^n max(0, 1- y_i ( \mathbf{w} \mathbf{x}_i + b ) 
\end{equation}
...
The substraction of a negative number will result in a positive penalizization. 
...
but also by complex decision boundaries due to the kernel trick. 
