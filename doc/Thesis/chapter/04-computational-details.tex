...
\td{Multiple \gls{iv} curves were measured (where are they?)} for every sample.
...
The \textit{optimizands} where chosen to be \td{$\gamma$, $\rho$, $\lambda$ and $v_{cal}$, (here it would help me to remind the reader what the symbols are!)}
...
The \td{experiments} are executed, \td{measured (unclear!)} and evaluated according to section 
\ref{sec:exp} and the responses conducticity $\gamma$ (see eq. \ref{eq:gamma}), pinhole density $\rho_s$ (see eq. \ref{eq:rho}), 
\todo{$\lambda$ (numbers of layers) (tstill not clear! what is meant?!)} and $v_{cal}$ (heating rate of calcination process in 
...
The population for the next time step is generated and the experiments are again \td{executed, measured and evaluated. (unclear!)}
...
Further hyper parameters were \td{$C$, $\varepsilon$ and $\gamma_{ml}$. 
$C$ and $\varepsilon$ (define again, forgotten until now) } took the values 0.05--1 with step size 0.05 for \gls{svm} and \gls{krr}.
\Gls{svm} additionally used the \texttt{scale} for $\gamma_{ml}$.
%\td{and was the same as for \gls{krr} plus \texttt{scale}.} %$\gamma_{ml}$ 0.01, 0.05, 0.1, 0.5, 1, 5, 10, and \texttt{scale}. 
